package com.lzb.employee.docs.config.swagger;
...
public class ServiceDescriptionUpdater {
	...
	public void refreshSwaggerConfigurations(){
		...
		for(int i = 0; i < discoveryClient.getServices().size(); i++){
			logger.info("count Instances "+discoveryClient.getServices().get(i));
			logger.info("desc: "+discoveryClient.description());
			
			
			logger.info("serviceInstance in Loop: "+discoveryClient.getInstances(discoveryClient.getServices().get(i)));
		}
		...
		discoveryClient.getServices().stream().forEach(serviceId -> {
			...
			List<ServiceInstance> serviceInstances = discoveryClient.getInstances("123");
			logger.info("serviceInstance "+serviceInstances.toString());
